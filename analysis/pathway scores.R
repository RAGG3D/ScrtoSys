...
map(list("KICH", "READ", 
    ...
    function(cancer0){
      ...
      map(as.list(samplelist), function(sample1){
        ...
        as.data.frame(a$test)%>%
          write.csv(paste0("C:/Rstudio/Chapter 3/output/TCGA pathway scores/Hallmarks/",cancer0,"/", sample1, ".csv"), row.names = F)
      }) 
    })
...
map(list("KICH", "READ", 
    ...
    function(cancer0){
      ...
      b <- map_dfr(as.list(samplelist), function(sample1){
        read_csv(paste0("C:/Rstudio/Chapter 3/output/TCGA pathway scores/Hallmarks/",cancer0,"/", sample1, ".csv")) %>% 
          mutate(sample_ct = sample1) %>%
          dplyr::select(sample_ct, ID, enrichmentScore) %>%
          pivot_wider(names_from = ID, values_from = enrichmentScore)
      })
      ...
      a <- reshape::melt(cor(x %>% dplyr::select(-sample_ct))[-c(1:2330), c(1:2330)]) 
      a.p <- rcorr(as.matrix((x %>% dplyr::select(-sample_ct)))) 
      a %>% inner_join(as.data.frame(a.p$P[-c(1:2330), c(1:2330)]) %>% 
                         mutate(X1 = rownames(as.data.frame(a.p$P[-c(1:2330), c(1:2330)]))) %>% 
                         gather(X2, p.value, -X1)) %>%
        write.csv(paste0("C:/Rstudio/Chapter 3/output/TCGA pathway scores/Hallmarks/tumor/", cancer0, " pathway+scrto corr.csv"), row.names = F)
...
map(list("KICH", "READ", 
    ...
    function(cancer0){
      ...
      a %>% inner_join(as.data.frame(a.p$P[-c(1:2330), c(1:2330)]) %>% 
                         mutate(X1 = rownames(as.data.frame(a.p$P[-c(1:2330), c(1:2330)]))) %>% 
                         gather(X2, p.value, -X1)) %>%
        write.csv(paste0("C:/Rstudio/Chapter 3/output/TCGA pathway scores/Hallmarks/normal/", cancer0, " pathway+scrto corr.csv"), row.names = F)
